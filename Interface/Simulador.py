...
def carrega_parametros():
    ...
    :rtype : pd.DataFrame
    """
    parametros = pd.read_csv("Interface/alphas.csv", index_col=1, header=0)
    #print(parametros[:10])
    return parametros
...
def distribui_notas(ncorte, nneg, ntoi, processos):
    ...
    # No cálculo de só Toi precisamos descontar só toicorte, só toineg e tcn
    processos["socortes_apl"] = ncorte * PREVALENCIAS.corte - (processos.sotc_apl+processos.socn_apl+processos.tcn_apl)
    # No cálculo de só Neg precisamos descontar só corteneg, só toineg e tcn
    processos["sonegs_apl"] = nneg * PREVALENCIAS.negativacao - (processos.sotn_apl+processos.socn_apl+processos.tcn_apl)
...
def simula(horizonte=30, ntoi=0, ncorte=0, nneg=0):
    ...
    processos['Novos'] = total_notas-total_notas*np.exp(-horizonte * (PARAMETROS.alfa0 + PARAMETROS.c*processos.cortes_apl +\
